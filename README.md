...
├── wrangler.jsonc              # Committed Workers config — see "Deployment" below
├── workers/
│   └── redirect-www.js        # Thin Worker in front of the static assets: www → apex, else pass through
├── tsconfig.json              # TypeScript with @ path aliases
├── .env.example               # Document required env vars
├── package.json
├── scripts/
│   └── generate-raster-assets.mjs # SVG → PNG social cards + right-sized wordmarks
├── public/
│   ├── _headers               # Cache and security headers, read by the Worker's static-asset handler
│   ├── .well-known/security.txt # RFC 9116 security contact
│   ├── favicon.png            # Browser tab icon (v4 "CdX" mark)
│   ├── robots.txt
│   ├── llms.txt               # LLM-readable site summary
│   ├── brand/logo/v4/         # Official v4 brand pack (wordmark, icon, favicons)
│   │                          # incl. generated -h32/-h64 wordmark variants
│   └── og/cloudox-og.{svg,png} # Default social card (SVG master, PNG served)
├── docs/
...
└── src/
    ...
    ├── config/example.ts      # Facts quoted from the sanitized example report
    ├── layouts/BaseLayout.astro
    ├── layouts/LegalLayout.astro # Shared shell for imprint/privacy/terms
    ├── components/            # Header, Footer, Hero, CTA, Section, FeatureCard,
    │                          # FeatureGrid, CodeBlock, Logo, ConfidenceBadge,
    │                          # EnvironmentSnapshot, AudienceViews
    ...
    └── pages/
        ...
        ├── thanks.astro       # No-JS form success page (noindex, off-sitemap)
...
| `npm run assets`  | Regenerate raster assets (PNG social cards + right-sized wordmarks) |
| `npm run assets:check` | Fail if a generated raster asset is missing or stale |
...
- Default OG image: `public/og/cloudox-og.png` (override with the `image`
  prop on `BaseLayout`). The SVG next to it is the master; the PNG is what
  ships, because no major social platform renders an SVG social card. Run
  `npm run assets` after editing any card SVG.
- Exactly one `og:type` is emitted per page, chosen with `BaseLayout`'s
  `ogType` prop (`article` for blog posts). Pages must not add their own.
- `www.cloudox.io` is permanently redirected to the apex by `workers/redirect-www.js`,
  so the canonical host is the only addressable one. This lives in the Worker
  rather than a `_redirects` rule because Cloudflare Workers Static Assets only
  accepts relative (same-host) rules there — a host-changing redirect has to be
  code. See **Deployment** below.
...
## Product proof (the sanitized example)
...
The dashboard screenshot (`public/screenshots/operations-view.png`) is a real
capture of `preview.cloudox.io`, not an illustration — regenerate it with
Playwright against the live preview rather than hand-editing it, and keep it
resized/compressed (`sharp`, ~1600px wide) the same way `npm run assets`
handles the other raster assets.

Rules for the `example.ts` module:

- **Quote, never round or improve.** If the example says six of seven
  accounts were in scope, the site says six of seven — including the caveats
  (excluded account, unavailable collectors, lower-bound counts). The point
  of the section is that the product states its own limits.
- **Carry the confidence label with the fact.** `ConfidenceBadge.astro`
  renders the product's four user-facing labels and nothing else, and the
  label is always written out, never colour alone.
- **Re-verify after every republish.** When `cloudox publish-example` emits
  a new bundle, re-read the example's own headline figures and update
  `EXAMPLE.generatedOn` with them. `tests/site-build.test.ts` asserts the
  figures and caveats are present in the built HTML, so a stale rewrite that
  drops them fails the suite — but it cannot tell you a number changed.
...
- `tests/site-build.test.ts` — assertions against the generated `dist/`
  ...
  `/.well-known/security.txt` is present with the required fields. It also
  covers the correctness and no-JavaScript guarantees that are easy to
  regress silently: one `og:type` per page and a raster social card, the
  `www` → apex redirect, the header's mobile CTA and menu accessibility
...
### Why there's a committed `wrangler.jsonc`

This project's deploy command is `npx wrangler deploy` (Cloudflare's unified
Workers & Pages path for the Astro preset), which deploys to **Workers Static
Assets**, not classic Pages. Without a committed Wrangler config, that command
runs an interactive first-time setup on every build — which auto-installs the
`@astrojs/cloudflare` SSR adapter, switches the build to `output: "server"`,
and adds a KV session binding this static site never needs.

`wrangler.jsonc` + `workers/redirect-www.js` pin that down instead: the build
stays a plain `astro build` (`output: "static"`), and the deployed Worker is a
few lines that redirect `www` → apex and otherwise hand every request straight
to `env.ASSETS.fetch()` (the static files in `dist/`). Workers Static Assets'
`_redirects` file only supports same-host, relative rules — an absolute-URL
"from" like the old `https://www.cloudox.io/*` is rejected at deploy time with
"Only relative URLs are allowed" (error 100324) — which is why the host
redirect is a Worker instead of a `_redirects` line.
...
The form is a plain `POST` to Web3Forms with a hidden `redirect` field
pointing at `/thanks`, so it works with JavaScript disabled or broken —
browser validation, a real submission, and a confirmation page. When
JavaScript is available it intercepts the submit, drops the `redirect`
field, and confirms in place instead. Keep both paths working: if you change
the form, test it once with JavaScript disabled.
...
- **Do not** state a figure about a real environment outside
  `src/config/example.ts`, and do not drop the caveat that comes with it. A
  number without its basis is the one thing this product exists to prevent.
- **Do not** describe direction as shipped. Continuous scheduling, Azure and
  GCP collectors, and the assistant are roadmap; AWS discovery is not. Check
  `cloudox-repo/docs/11-roadmap.md` before promising a capability.
